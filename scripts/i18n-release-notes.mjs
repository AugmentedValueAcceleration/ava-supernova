...
const RELEASES = [
  {
    ...
    platform: 'extension',
    toolCount: 121,
    publishedAt: '2026-09-10 22:45:00+00',
...
**Closing the editor threw the session away.** Reflection — the pass where I look back over what we did and keep what matters — only ran when you started a new chat, switched conversation, or changed project. Shutting down reached none of those. So an evening of real work, ended the normal way by closing the window, left nothing behind. And it could not have been fixed by running it at shutdown either: that work is a model call, and it would have been killed mid-sentence along with everything else.

**So it no longer waits for a tidy ending.** When a session goes quiet for a while, I write it down. If you come back and carry on, that is cancelled and it waits again. Whether you close the editor properly, or just close the laptop, the evening is kept.
...
The desktop IDE had the same gap and got the same fix.`,
    highlights: [
      'Closing the editor used to lose the whole session — reflection only ran on a new chat, a conversation switch, or a project change.',
