...
- **She picks the right brain** — three orchestrated modes ship in the dropdown. **Maestro** runs a single-coordinator pipeline (Qwen 3.6 Plus, the Terminal-Bench leader). **Supernova** *(in development)* extends to polyglot multi-model orchestration — DeepSeek V4 Pro coordinator + Qwen 3.6 Plus Builder + V4 Flash for review + Omni for vision. **Aurora** *(in development)* is the European AI stack — Mistral Large 3 coordinator + Mistral Small 4 specialists, end-to-end. Never leaves European infrastructure. Apache-2.0, open weights, sovereign by design.
