...
export function Header({
  ...
  return (
    <div className="flex items-center gap-1 px-3 py-2 border-b" style={{ borderColor: 'rgba(168, 85, 247, 0.12)' }} role="toolbar" aria-label="Chat controls">
      ...
      {/* History */}
      <button onClick={onOpenHistory} title={t('header.history') === 'header.history' ? 'History' : t('header.history')} aria-label="History" className={btnBase}>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
      ...
      <button
        ...
        title={t('header.tasks') === 'header.tasks' ? 'Tasks' : t('header.tasks')}
        aria-label="Tasks"
      ...
      {/* New Chat */}
      <button onClick={onNewChat} title={t('header.new_chat') === 'header.new_chat' ? 'New chat' : t('header.new_chat')} aria-label="New chat" className={btnBase}>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
