...
export function TodoCard({ toolCall, isLatest = true, isStreaming = false }: TodoCardProps) {
  ...
  if (!todos) {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] px-3 py-2 text-xs opacity-60">
        {t('todo.unavailable')}
      </div>
    );
  }
  ...
  return (
    ...
    >
      <button
        ...
        className="flex w-full cursor-pointer items-center gap-2.5 border-none bg-transparent px-3 py-2 text-left transition-colors hover:bg-[var(--bg-hover)]"
      >
        {/* A ring, not a tick in a box — the same shape the collapsed tasks
            rail uses, so the two read as one idea at two sizes. */}
        <span className="relative flex h-[22px] w-[22px] flex-shrink-0 items-center justify-center">
          ...
          {allDone && (
            <span className="absolute text-[10px] font-semibold" style={{ color: accent }}>✓</span>
          )}
        </span>
      ...
      {expanded && (
        <ul
          className="list-none space-y-0.5 border-t px-3 pb-2.5 pt-2"
          ...
          {todos.map((todo, i) => {
            ...
            return (
              <li key={i} className="flex items-start gap-2.5 py-[3px]">
                ...
                <span
                  className={`text-[11.5px] leading-[1.5] ${todo.status === 'completed' ? 'text-[var(--text-muted)] line-through' : 'text-[var(--text-secondary)]'}`}
