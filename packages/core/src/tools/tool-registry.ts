...
import { ProposeCoursesTool, FindCourseTool, ReadCourseTool, WriteCourseTool, ReviseCourseTool, CheckCourseTool, RegenerateCoverTool, TranslateCourseTool, ProposeCategoryTool } from './course-tools.js';
...
export class ToolRegistry {
  ...
  registerBuiltins(options?: { exclude?: string[]; allowDesktopMode?: boolean }): void {
    ...
    const builtins: Tool[] = [
      ...
      new ReviseExerciseTool(),
      new AddEquipmentTool(),
      new AddContraindicationTool(),
      new ProposeConditionTool(),
      new SetMusclesTool(),
      new RegenerateDemoTool(),
      new CheckExerciseTool(),
      new ProposeExerciseSeedsTool(),
      // The Classroom — Ava as tutor and course author. Same shape again.
      new ProposeCoursesTool(),
      new FindCourseTool(),
      new ReadCourseTool(),
      new WriteCourseTool(),
