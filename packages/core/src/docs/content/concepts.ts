...
export const CONCEPT_PAGES: DocPage[] = [
  {
    ...
    deeper: [
      ...
      { type: 'table',
        ...
        rows: [
          ['Coordinator',     'Qwen 3.7 Plus',           'DeepSeek V4.1 Flash (1M ctx, 384K max output, multimodal)',  'Mistral Medium 3.5 (128B dense, 256K ctx)'],
          ['Builder',         'Qwen 3.7 Plus (reuses coordinator)', 'Qwen 3.7 Plus',                     'Mistral Medium 3.5 (128B dense, 256K ctx)'],
          ['Mid-tier specialists', '—',                    'DeepSeek V4.1 Flash (code reviewer, fact checker, recon, scanner, security verifier, etc.)', 'Mistral Small 4 (Verifier, Sequencer, recon, scanner, reporter)'],
          ['Intent gate',      'Qwen 3.7 Flash',           'Qwen 3.7 Flash',                              'Mistral Small 4'],
          ['Light specialists', 'Qwen 3.5 Flash',          'Qwen 3.5 Flash (Scout, Verifier, Sequencer, Challenger, Integrator)',                      'Mistral Small 4'],
          ['Heavy specialists', '—',                       '—',                                          'Mistral Medium 3.5 (Researcher, Challenger, CVE Researcher, Fact Checker, Security Verifier)'],
          ['Chat',            'Qwen 3.5 Flash',           'DeepSeek V4.1 Flash',                      'Mistral Small 4'],
          ['Image-gen orchestration', 'Qwen 3.5 Flash',  'Qwen 3.5 Flash',                              'Mistral Small 4'],
          ['Vision',          'Qwen 3.7 Plus (native)',   'Qwen 3.8 Flash (native, image + video)',      'Mistral Medium 3.5 (vision encoder trained from scratch)'],
      ...
      { type: 'paragraph', text: 'Tier-differentiated Qwen routing, light footprint. Qwen 3.7 Plus owns the coordinator, Builder, planning, security, brainstorm, and long-context work — the slots where its 1M-context hybrid linear-attention pays off. Qwen 3.5 Flash handles chat and image-gen orchestration, where its $0.05/$0.40 pricing wins on the bulk of low-depth token volume. The upstream intent gate moved to Qwen 3.7 Flash on 2026-09-10 — measured at 99.6% against 3.5 Flash’s 92.9% for 32ms more at p50, and a miss there spawns a whole specialist team rather than merely answering less well. Qwen 3.7 Plus also reads images and video natively, so vision input needs no separate model. The coordinator falls through the ladder Qwen 3.7 Plus → 3.8 Flash → 3.5 Flash if the primary is unavailable. Qwen 3.5 Plus left that ladder on 2026-09-10: Qwen 3.8 Flash beat it on every axis at once — cheaper, multimodal, a generation newer — so there was no longer any job for which it was the right rung. It stays selectable; it is simply no longer a step the fleet walks through.' },
      { type: 'heading', level: 3, text: 'Supernova' },
      { type: 'paragraph', text: 'Polyglot ensemble. DeepSeek V4.1 Flash coordinates and dispatches each subtask to its best-suited specialist: Qwen 3.7 Plus runs every Builder spawn, DeepSeek V4.1 Flash handles chat plus planning, security, teach and the mid-tier review/audit personas, Qwen 3.5 Flash absorbs the light-tier classifier work and orchestrates image-gen tool calls, Qwen 3.8 Flash reads vision input, and the long-form Content Writer output runs on Qwen 3.7 Plus. Vision is worth a note: it sits outside the DeepSeek family by CHOICE now rather than necessity. Both of V4.1 Flash\'s predecessors were blind at the API level and vision had to leave; V4.1 Flash sees perfectly well, and the seat stays with Qwen because that is what our vision paths are tuned against and Qwen takes video too. The whole fleet now runs at flash-tier prices without giving up frontier-class work — which is less of a trade than it sounds, because on both of these vendors the flash line is where the newest model lands first.' },
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'paragraph', text: 'Per-action cost table — what gets deducted from your credit pool when each action runs. (A "cache hit" is when Ava reuses work it already did instead of redoing it; when that happens you pay less — the charge drops by 70%, or 50% on output-heavy models like DeepSeek Flash.)' },
      { type: 'list', ordered: false, items: [
      ...
      { type: 'paragraph', text: 'Each model carries a multiplier that scales an action\'s cost to track what that model actually costs to run — lighter models cost less than the flat bracket, heavier ones more. Calibrated for ~30% net margin, rebalanced down from 55% and then from 40%: each time the target drops, every multiplier drops with it and the difference goes back to you.' },
      { type: 'list', ordered: false, items: [
        '6.24× — Kimi K3 (Longxiang\'s coordinator and Builder), at $3.00/$15.00. The priciest model we serve, and the priciest multiplier — that is real cost, not margin.',
        '3.87× — Mistral Medium 3.5 (Aurora\'s lead coordinator), at its $1.50/$7.50 frontier-flagship rate. This is the model the whole table is calibrated against.',
        '3.22× — Qwen 3.7 Max.',
        '1.68× — Kimi K2.7 Code.',
        '1.36× — Qwen 3.8 Max. Newer and better than 3.7 Max and less than half the multiplier, because Alibaba cut the price — that is not how this usually goes, so it is worth saying out loud.',
        '0.94× — Qwen 3.7 Plus (Maestro coordinator, and Supernova\'s Builder).',
        '0.92× — Mistral Large 3 (Aurora\'s heavy reserve).',
        '0.85× — Mistral Small 4 (Aurora\'s high-volume workhorse), at its real $0.15/$0.60 rate.',
        '0.44× — DeepSeek V4.1 Flash (Supernova\'s coordinator). It fell from 1.35× when DeepSeek retired V4 Pro into V4.1 Flash on 10 September 2026: the same seat, a better model, and a third of the price.',
        '0.44× — Qwen 3.8 Flash and Qwen 3.7 Flash. Both INTERIM ceilings rather than measured figures — they are pitched at DeepSeek\'s measured rate, which their prices cannot exceed, until our own traffic replay gives a real number. Expect these to fall.',
        '0.44× — Qwen 3.5 Plus.',
        '0.44× — Qwen 3.7 Flash (every fleet\'s intent gate since 10 September 2026 — 99.6% accurate against 3.5 Flash\'s 92.9% over 280 measured calls, for 32ms).',
        '0.16× — Qwen 3.5 Flash (the light specialists, and the gate\'s fallback).',
        '1.0× — any model without a listed multiplier. Base rate.',
        'One note on DeepSeek: it charges double between 01:00–04:00 and 06:00–10:00 UTC. Every DeepSeek price quoted here and in the cost calculator is the off-peak one, which applies for 17 hours of every 24.',
      ]},
