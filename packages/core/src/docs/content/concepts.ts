...
export const CONCEPT_PAGES: DocPage[] = [
  {
    ...
    body: [
      ...
      { type: 'paragraph', text: 'Two-model system, light footprint. Production-tuned, proven across months of operator workloads, the default on every plan.' },
      { type: 'heading', level: 4, text: 'Coordinator' },
      { type: 'paragraph', text: 'Qwen 3.6 Plus — 1M context, native function calling, agentic-coding leader. Drives the conductor brain plus Builder spawns (Builder reuses the coordinator in Maestro mode).' },
      { type: 'heading', level: 4, text: 'Light tier' },
      { type: 'paragraph', text: 'Qwen 3.5 Flash — handles the upstream intent gate (decides whether the coordinator can answer directly or whether the persona team is genuinely needed) and any light-tier Conductor personas (the Conductor receives Flash as its lightModel pair). Keeps the cheap operations cheap so the bulk of token volume runs at flash pricing.' },
      { type: 'heading', level: 4, text: 'Fallback ladder' },
      { type: 'paragraph', text: 'Qwen 3.6 Plus → Qwen 3.5 Plus → Qwen 3.5 Flash. If the primary coordinator is unavailable on your plan, Ava falls through the priority list rather than failing.' },
      { type: 'heading', level: 4, text: 'Data residency' },
      { type: 'paragraph', text: 'Mixed — wherever Alibaba Cloud hosts Qwen.' },
      { type: 'heading', level: 4, text: 'Best for' },
      { type: 'paragraph', text: 'Daily work, predictable cost, anyone who wants Ava to "just work" without thinking about routing.' },

      { type: 'heading', level: 3, text: 'Supernova' },
      { type: 'paragraph', text: 'Polyglot ensemble. The coordinator picks the best specialist for each task, model by model. Frontier reasoning where it matters, flash-tier economics on the bulk of token volume.' },
      { type: 'heading', level: 4, text: 'Coordinator' },
      { type: 'paragraph', text: 'DeepSeek V4 Pro — 1.6T total parameters / 49B active, 1M context, frontier tool-call reliability. Handles task classification, planning, long-context synthesis, and direct chat.' },
      { type: 'heading', level: 4, text: 'Builder' },
      { type: 'paragraph', text: 'Qwen 3.6 Plus — Terminal-Bench leader, production-tested. Every TaskExecutor spawn lands here.' },
      { type: 'heading', level: 4, text: 'Mid-tier specialists' },
      { type: 'paragraph', text: 'DeepSeek V4 Flash — code reviewer, fact checker, quiz master, recon, scanner, security verifier, security reporter, curriculum architect, explorer, refiner. Reasoning depth 3 at flash-tier cost.' },
      { type: 'heading', level: 4, text: 'Light specialists' },
      { type: 'paragraph', text: 'Qwen 3.5 Flash — Scout, Verifier, Sequencer, Challenger, Integrator. Reasoning depth ≤ 2, cheapest classifier in the roster.' },
      { type: 'heading', level: 4, text: 'Vision' },
      { type: 'paragraph', text: 'Qwen 3.5 Omni Plus — only vision-capable model in the Supernova table. Any prompt with images bypasses category routing and lands here.' },
      { type: 'heading', level: 4, text: 'Long-form writing' },
      { type: 'paragraph', text: 'Qwen 3.5 Plus — Content Writer persona for cost-sensitive long output.' },
      { type: 'heading', level: 4, text: 'Data residency' },
      { type: 'paragraph', text: 'Mixed — DeepSeek + Alibaba Cloud infrastructure.' },
      { type: 'heading', level: 4, text: 'Best for' },
      { type: 'paragraph', text: 'Heavy multi-step work where each subtask wants its own specialist. Frontier coordinator on every plan.' },

      { type: 'heading', level: 3, text: 'Aurora' },
      { type: 'paragraph', text: 'European AI stack — sovereign by design. Mistral-only routing, never leaves EU infrastructure. Apache-2.0 open weights.' },
      { type: 'heading', level: 4, text: 'Coordinator' },
      { type: 'paragraph', text: 'Mistral Large 3 — sparse MoE, 41B active / 675B total, 262K context. Handles classification, planning, long-context synthesis, chat.' },
      { type: 'heading', level: 4, text: 'Heavy specialists' },
      { type: 'paragraph', text: 'Mistral Large 3 — Researcher, Challenger, CVE Researcher, Ideator, Plan Researcher, Plan Challenger, Brainstorm Researcher, Brainstorm Challenger, Fact Checker, Security Verifier. Depth-heavy roles where reasoning ceiling matters more than throughput.' },
      { type: 'heading', level: 4, text: 'Builder + light specialists' },
      { type: 'paragraph', text: 'Mistral Small 4 — Architect, Builder, Verifier, Sequencer, Refiner, Tutor, Curriculum Architect, Explorer, Recon, Scanner, Reporter. Devstral lineage plus Pixtral vision merged into one model at flash-tier cost ($0.15/$0.60 per 1M).' },
      { type: 'heading', level: 4, text: 'Vision' },
      { type: 'paragraph', text: 'Mistral Small 4 — Pixtral capability is baked in, so attaching an image does not force a model switch. Aurora is unique in not needing a separate vision specialist.' },
      { type: 'heading', level: 4, text: 'Intent gate' },
      { type: 'paragraph', text: 'Mistral Small 4 — same model as the Builder. Using one model for the gate plus Builder gives a tighter cache-hit story than swapping models for the classification call.' },
      { type: 'heading', level: 4, text: 'Data residency' },
      { type: 'paragraph', text: 'EU only — sovereign-stack guarantee. Aurora deliberately does not silently cross-route to a non-Mistral model when one is unavailable; the router surfaces an error instead. That is what makes it "Aurora" rather than a Mistral-flavoured Maestro.' },
      { type: 'heading', level: 4, text: 'Best for' },
      { type: 'paragraph', text: 'GDPR-strict deployments, public-sector and healthcare buyers, anyone with a sovereignty mandate.' },
