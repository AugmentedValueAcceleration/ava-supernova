...
- "minimize_all" — minimize EVERY window to reveal the desktop and its icons (Recycle Bin, This PC, files, shortcuts). No target/params. Desktop icons live on the desktop, NOT inside any app window, so they only appear in the element list once the desktop is showing. Use this FIRST whenever the task targets a desktop icon, or the element you need clearly isn't in the visible list because a window is covering the desktop.
...
6b. OCCLUDED ELEMENTS CANNOT BE CLICKED. An element with occluded: true is physically covered by another window — a click would hit the covering window, not the element (the screen can LIST desktop icons even when an app is sitting on top of them). Before targeting an occluded element: "minimize_all" for desktop icons, or focus the window the element belongs to. Never click through an occlusion.
...
3a. SILENT SUCCESS IS SUCCESS. Many Windows actions complete with NO visible confirmation: emptying the Recycle Bin shows no message, a dialog's Yes/OK simply closes the dialog, minimize shows no banner. If the ExecutionResult is ok and nothing in the fresh state POSITIVELY CONTRADICTS the expected outcome, that is 'verified'. The ABSENCE of a confirmation message, dialog, or "indicator" is never, by itself, a deviation. Demand contradiction, not fanfare.
3b. The fresh ScreenState is one snapshot, possibly taken mid-animation — context menus, dialogs and window transitions take a few hundred ms to render, and a dialog CLOSING legitimately drops the element count. A missing transient surface or a count change consistent with the expected transition ("dialog closed" → fewer elements; "menu opened" → same elements until it renders) supports 'verified', not 'deviated'. Reserve 'deviated' for evidence of the WRONG thing happening.
3c. Elements carry a 'surface' field — "menu" means an OPEN context menu/dropdown is on screen (do not misread its items as search results or a list view), "desktop-icon" means the desktop's icon list, "app" means a normal window. If the prediction was "a context menu appears" and the fresh state contains surface:"menu" elements, that IS the menu: 'verified'.
