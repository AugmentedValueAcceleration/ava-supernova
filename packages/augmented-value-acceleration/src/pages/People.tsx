  ...
  return (
    <div style={{ padding: '40px 48px', overflowY: 'auto', height: '100%', background: '#0a0a1a' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#fff', margin: 0 }}>People</h1>
          <p style={{ fontSize: 14, color: '#6b7280', marginTop: 4 }}>Your team directory</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'Total Team', value: stats.total, icon: '👥', color: '#a855f7' },
      ...
      {/* Loading */}
      {loading && <div style={{ textAlign: 'center', color: '#6b7280', padding: 60 }}>Loading team...</div>}

      {/* Empty */}
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: 80, color: '#6b7280' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>👥</div>
          <div style={{ fontSize: 16, fontWeight: 600, color: '#9ca3af', marginBottom: 8 }}>No team members found</div>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 14 }}>
                  <div style={{ position: 'relative' }}>
                    {p.avatar_url ? (
                      ...
                      <div style={{
                        width: 48, height: 48, borderRadius: '50%', background: '#1f1f3a',
                  ...
                  <div>
                    ...
                    {p.title && <div style={{ fontSize: 12, color: '#9ca3af' }}>{p.title}</div>}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  ...
                  {p.department && <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, background: '#1f1f3a', color: '#9ca3af' }}>{p.department}</span>}
                </div>
                ...
                {expanded && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #1f1f3a' }}>
                    <div style={{ fontSize: 12, color: '#9ca3af', display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
                    ...
                    {p.skills && p.skills.length > 0 && (
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>Skills</div>
                    ...
                    {p.projects_assigned && p.projects_assigned.length > 0 && (
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>Projects</div>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {p.projects_assigned.map((proj, i) => (
                            <span key={i} style={{ padding: '2px 10px', borderRadius: 6, fontSize: 11, background: '#1f1f3a', color: '#9ca3af' }}>{proj}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    <button onClick={e => { e.stopPropagation(); handleDelete(p.id); }} style={{
                      padding: '6px 16px', background: '#7f1d1d', color: '#fca5a5',
      ...
      {showCreate && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, color: '#fff', margin: '0 0 24px' }}>Add Person</h2>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: 12, color: '#9ca3af', display: 'block', marginBottom: 6 }}>Display Name *</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: '#9ca3af', display: 'block', marginBottom: 6 }}>Skills (comma separated)</label>
