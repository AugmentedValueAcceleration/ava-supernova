...
const PIPELINE_STAGES = [
  ...
  { key: 'qualified', label: 'Qualified', color: theme.accent },
...
export default function CRM() {
  ...
  return (
    <div style={pageStyle}>
      ...
      {/* Loading */}
      {loading && <div style={{ textAlign: 'center', color: theme.textMuted, padding: 60 }}>Loading CRM data...</div>}
            ...
            return (
              <div key={stage.key} style={{ background: theme.surfaceBg, borderRadius: 14, padding: 14, border: '1px solid #1f1f3a' }}>
                <div style={{ marginBottom: 14, paddingBottom: 10, borderBottom: `2px solid ${stage.color}` }}>
                  ...
                  {stageValue > 0 && (
                    <div style={{ fontSize: 11, color: theme.textMuted, marginTop: 4 }}>${stageValue.toLocaleString()}</div>
                  )}
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {stageClients.length === 0 && (
                    <div style={{ fontSize: 11, color: theme.textMuted, textAlign: 'center', padding: 16 }}>No clients</div>
                  )}
                  {stageClients.map(c => {
                          ...
                          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #1f1f3a' }}>
                            ...
                            <div style={{ display: 'flex', gap: 4 }}>
                              <button onClick={e => { e.stopPropagation(); setShowContact(c.id); }} style={{
                                padding: '3px 10px', background: theme.border, color: theme.accent, border: 'none',
                                borderRadius: 6, fontSize: 10, cursor: 'pointer',
                              }}>+ Contact</button>
                              <button onClick={e => { e.stopPropagation(); handleDelete(c.id); }} style={{
                                padding: '3px 10px', background: theme.redBg, color: theme.red, border: 'none',
                                borderRadius: 6, fontSize: 10, cursor: 'pointer',
                              }}>Delete</button>
                            </div>
                          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Name *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Company</label>
                  <input style={inputStyle} value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} placeholder="Company name" />
                </div>
                <div>
                  <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Website</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Source</label>
                  <input style={inputStyle} value={form.source} onChange={e => setForm({ ...form, source: e.target.value })} placeholder="Referral, cold, inbound..." />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Deal Value</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Notes</label>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Subject *</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: theme.textSecondary, display: 'block', marginBottom: 6 }}>Content</label>
